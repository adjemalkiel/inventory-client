...
import type { DjangoUser, StockMovement, StockMovementType } from '@/types/api';
...
function formatQtyDisplay(type: StockMovementType, qtyRaw: string): { text: string; className: string } {
...
export default function MovementsPage() {
  ...
  const [countPrev24h, setCountPrev24h] = useState<number | null>(null);

  const [weekBars, setWeekBars] = useState<
    { label: string; entree: number; sortie: number; transfert: number; retour: number }[]
  >([]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      apiServices.stockMovements.rawList({
        page_size: 1,
        created_at_after: dayAgo.toISOString(),
        created_at_before: now.toISOString(),
      }),
      apiServices.stockMovements.rawList({
        page_size: 1,
        created_at_after: twoDaysAgo.toISOString(),
        created_at_before: dayAgo.toISOString(),
      }),
    ])
      .then(([r24, r48]) => {
        setCount24h(isPaginatedResponse(r24) ? r24.count : r24.length);
        setCountPrev24h(isPaginatedResponse(r48) ? r48.count : r48.length);
      })
      .catch(() => {
        setCount24h(null);
        setCountPrev24h(null);
      });
  }, []);
  ...
  useEffect(() => {
    ...
    apiServices.stockMovements
      ...
      .then((resp) => {
        ...
        const bucket: Record<
          string,
          { entree: number; sortie: number; transfert: number; retour: number }
        > = {};
        for (const { key } of meta) {
          bucket[key] = { entree: 0, sortie: 0, transfert: 0, retour: 0 };
        }
        for (const m of list) {
          const k = localDateKey(m.created_at);
          if (!bucket[k]) continue;
          const t = m.movement_type;
          if (t === 'entree' || t === 'sortie' || t === 'transfert' || t === 'retour') {
            bucket[k][t] += 1;
          }
        }
        setWeekBars(
  ...
  }, [page, dateFrom, dateTo, userFilter, typeFilter]);
  ...
  const pctVsPrev =
    count24h !== null && countPrev24h !== null && countPrev24h > 0
      ? Math.round(((count24h - countPrev24h) / countPrev24h) * 100)
      : null;

  const maxWeekDayTotal = useMemo(() => {
    let m = 1;
    for (const d of weekBars) {
      const s = d.entree + d.sortie + d.transfert + d.retour;
      if (s > m) m = s;
    }
    return m;
  }, [weekBars]);
              ...
              <div className="mt-4 flex items-center gap-2 text-[10px] font-bold uppercase">
                {pctVsPrev !== null ? (
                  <>
                    ...
                    <span className={pctVsPrev >= 0 ? 'text-green-400' : 'text-amber-400'}>
                      {pctVsPrev >= 0 ? '+' : ''}
                      {pctVsPrev}% vs fenêtre précédente
                    </span>
                  </>
                ) : (
                  <span className="text-white/50">Pas assez de données pour la tendance</span>
                )}
              </div>
            </div>
              ...
              <table className="min-w-[800px] w-full border-collapse text-left">
                <thead>
                  <tr className="bg-surface-container-high/40">
                    <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-slate-600">
                      Date & Heure
                    </th>
                    <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-slate-600">
                      Type
                ...
                <tbody className="divide-y divide-slate-100/50">
                  {loading ? (
                    ...
                    movements.map((m) => {
                      ...
                      const qtyFmt = formatQtyDisplay(m.movement_type, m.quantity);
                      const { source, dest } = sourceDestLabels(m);
                      const when = new Date(m.created_at);
                          ...
                          <td className="px-6 py-5">
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex min-w-0 flex-col">
                                ...
                                {m.item_sku?.trim() ? (
                                  <span className="text-[11px] text-slate-400">Réf: {m.item_sku}</span>
                                ) : null}
                              </div>
                            </div>
                          </td>
